...
from bokeh.models import LinearColorMapper, ColorBar, ColumnDataSource, Range1d, CustomJS, Div, \
    CDSView, \
    IndexFilter, BooleanFilter, Span, Label, BoxZoomTool
...
class astro_web(object):
    def __init__(self, data_type, data_path):
        ...
        self.umap_data = get_umaps('galaxies/umaps')
    ...
    def generate_buttons(self):
        ...
        self.select_score = Dropdown(label="Color by:", button_type="danger", menu=select_score_menu, value='Hd EW')
        ...
        self.anomaly_detection_algorithms = get_anomalies_dict()
        ...
        self.selected_galaxies_columns = [
            ...
            TableColumn(field="score", title="Color", formatter=NumberFormatter(format = '100.00')),
    ...
    def generate_figures(self):
        ...
        self.umap_figure = figure(tools='lasso_select,box_zoom,save,reset',
        ...
        self.spectrum_figure = figure(tools="box_zoom,save,reset", plot_width=1200, plot_height=320,
                                      toolbar_location="above", output_backend='webgl')

        self.stacks_figure = figure(tools="box_zoom,save,reset", plot_width=1200, plot_height=320,
                                      toolbar_location="above", output_backend='webgl')
        ...
        self.spectrum_figure.title = t

        self.spectrum_figure.xaxis.axis_label = 'Rest wavelength [A]'
        self.spectrum_figure.xaxis.major_label_text_font_size = "15pt"
        self.spectrum_figure.xaxis.axis_label_text_font_size = "15pt"
        self.spectrum_figure.yaxis.major_label_text_font_size = "15pt"
    ...
    def generate_sources(self):
        ...
        sd = get_score_data(self.select_score.value)
        ...
        self.spectrum_source = ColumnDataSource(
            data=dict(xs=self.wave,
                      ys=np.log(self.specs_gal[0] + sys.float_info.epsilon) if self.use_log() else self.specs_gal[0])
        )
    ...
    def set_colormap(self, sd):
        ...
        mn = np.nanmin(sd)
        if mn == mx:
            self.color_mapper.high = mx + 1
            self.color_mapper.low = mn - 1
        else:
            self.color_mapper.high = mx + (mx - mn)*self.high_colormap_factor
            self.color_mapper.low = mn
        print('set_colormap')
        print(self.color_mapper.high)
        print(self.color_mapper.low )
        return
    ...
    def update_spectrum(self):
        ...
        self.spectrum_source = ColumnDataSource(
            data=dict(xs=self.wave,
                      ys=np.log(self.specs_gal[int(index)] + sys.float_info.epsilon) if self.use_log() else
                      self.specs_gal[int(index)]))
